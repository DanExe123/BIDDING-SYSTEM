<div x-data="{
    ...
    openModal(id) {
        // find the notif in the Livewire notifications array
...
<!-- Dropdown -->
<div x-show="open" @click.away="open = false" x-transition class="absolute right-0 mt-2 w-96 bg-white border rounded-lg shadow-lg overflow-hidden z-50">
    <div class="p-4 border-b font-semibold text-gray-700">Notifications</div>

    <ul class="max-h-80 overflow-y-auto divide-y">
        @forelse($notifications as $notif)
        <li class="w-full px-4 py-3 flex items-start gap-3 rounded-md transition {{ !$notif['is_read'] ? 'bg-gray-100' : '' }}"
        @click="openModal({{ $notif['id'] }}); $wire.markSingleAsRead('{{ $notif['type'] }}', {{ $notif['id'] }})"
    >
        ...
        @empty
            <li class="px-4 py-3 text-center text-sm text-gray-500">No notifications found</li>
        @endforelse
    </ul>
</div>
