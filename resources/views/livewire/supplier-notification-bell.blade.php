<div x-data="{
    ...
    <div x-show="showAnnouncementModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 px-4" x-transition>
        <div class="bg-white rounded-xl shadow-xl w-full max-w-lg p-6 transform transition-all duration-300 scale-95"
            ...
            <!-- Body -->
            <div class="space-y-3">
                <p x-text="selectedAnnouncement ? selectedAnnouncement.description : ''" class="text-gray-700 text-sm leading-relaxed"></p>
                <p x-text="selectedAnnouncement ? new Date(selectedAnnouncement.created_at).toLocaleDateString() : ''" class="text-xs text-gray-500 italic"></p>
            </div>
